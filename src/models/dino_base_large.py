...
def get_args():
    ...
    parser.add_argument("--output_dir", type=str, default="./checkpoints_ssl_large", help="Where to save weights")
...
class SSLConfig:
    ...
    # --- Checkpointing ---
    output_dir: str = "./checkpoints_ssl_large"
    keep_last_k: int = 10
    save_freq: int = 1
...
class DINOLoss(nn.Module):
    ...
    def koleo_loss(self, student_output):
        """
        DINOv2 Regularization.
        FIX: Uses out-of-place masking to prevent 'one of the variables modified' runtime error.
        """
        # 1. Normalize
        ...
        # 3. Mask Diagonal (Self-distance) - THE CRITICAL FIX
        # We create a new tensor 'pdist_masked' instead of modifying 'pdist' in-place.
        # This keeps the gradient graph valid for accumulation.
        mask = torch.eye(pdist.shape[0], device=pdist.device, dtype=torch.bool)
        
        # 'masked_fill' (no underscore) returns a NEW tensor.
...
def train_one_epoch(student, teacher, teacher_without_ddp, loss_fn, data_loader, optimizer, 
    ...
    for it, images in enumerate(pbar):
        ...
        for i, param_group in enumerate(optimizer.param_groups):
            ...
            if i == 0: param_group["weight_decay"] = wd_schedule[cur_iter]
        ...
        with accelerator.accumulate(student):
            ...
            # Only Clip, Step, and EMA if we are synchronizing gradients (end of accumulation)
...
def main():
    ...
    print(f'Gradients iwll be accumulated for {cfg.grad_accum_steps} steps')
